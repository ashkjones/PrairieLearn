...
module.exports = {
  ...
  _makeAndInsertVariant(
    ...
    this._selectQuestion(question_id, instance_question_id, (err, question) => {
      if (ERR(err, callback)) return;
  ...
  saveAndGradeSubmission(submission, variant, question, course, overrideGradeRateCheck, callback) {
    ...
    async.series(
      [
        (callback) => {
          this.saveSubmission(submission, variant, question, course, (err, ret_submission_id) => {
            if (ERR(err, callback)) return;
  ...
  parseReadmeFile(locals){
    let filePath = `${locals.course.path}/questions/${locals.question.directory}/README.md`;
    let readData = "";
    ...
    fs.stat(filePath, function(err, stat){
        ...
        locals.readMeData = readData;
        locals.readMeDataExists = hasReadData;
